...
Use the `obsidian` CLI to interact with a running Obsidian instance. Requires
Obsidian to be open.
...
Commands target the most recently focused vault by default. Use `vault=<name>`
...
Use `--copy` on any command to copy output to clipboard. Use `silent` to prevent
